...
export class ScheduleGeneratorService {
    ...
    async generateProposal(request: ProposalRequest) {
        ...
        // We need to track what we've "booked" in this proposal to avoid self-collisions
        // (e.g. assigning two subjects to the same group at Mon 07:00)
        ...
        for (const subject of subjects) {
            ...
            if (blocksAssigned < blocksNeeded) {
                // Warn? Push a partial?
                // For now, we just do best effort.
            }
        }
